...
#
# Runs over both CARNIVAL variants. They share a PKN, a solver and its options;
# the measurement set is the only difference, so anything present in one network
# and absent from the other is attributable to the anchoring.
#
#   baseline - measurements were the top-n significant TFs only
#   anchored - the same TFs plus the forced CollecTRI regulators of HLH genes
#
...
build_layers <- function(tag, variant) {
  ...
  # Sign consistency is required, so an inferred-active repressor cannot be drawn
