# ============================================================
# 14 (anchored). Solved networks: inspection, pruning and downstream join
#
#     Identical to the baseline analysis, run on the HLH-anchored solves.
#     The measurement set for those runs was the top-n significant TFs plus
#     every expressed CollecTRI regulator of an HLH gene, forced in at its
#     observed score regardless of FDR. The baseline version of this script is
#     the comparator: same PKN, same solver, measurements the only difference.
#
...
# one CARNIVAL result object per contrast, from the anchored solves
carn <- lapply(setNames(tags, tags),
               function(tag) readRDS(sprintf("carnival_anch_%s.rds", tag)))

# measurement file used for those solves; referenced in the QC and comparison blocks
meas_file <- function(tag) sprintf("meas_%s_hlh_anchored.rds", tag)
...
build_layers <- function(tag) {
  ...
  # Under anchoring this set includes the forced HLH regulators.
  ...
  # Under anchoring, "unexplained" and "dropped" are expected for forced TFs
  # with near-zero scores, which contribute almost nothing to the objective.
  meas <- readRDS(meas_file(tag))
...
# Which forced TFs did the solver actually set? Forcing puts a TF in the
# objective; it does not guarantee the solution reaches it.
tf_aud <- readRDS("tf_aud.rds")

for (tag in tags) {
  forced <- tf_aud$source[tf_aud$contrast == tag]
  cat("\n", tag, " - forced HLH regulators:\n", sep = "")
  print(net[[tag]]$qc %>% filter(id %in% forced) %>%
          arrange(desc(abs(ulm))) %>% as.data.frame())
}
...
MIN_CONS <- 15 # same threshold as the baseline analysis, or the runs are not comparable
...
#     Filenames carry an _anch suffix so the baseline exports are not overwritten.
for (tag in tags) {
  for (v in c("pruned", "hlh", "full")) {
    x <- switch(v, pruned = pruned[[tag]], hlh = sub[[tag]], full = net[[tag]])
    write.table(x$edges, sprintf("edges_%s_%s_anch.tsv", tag, v),
                sep = "\t", row.names = FALSE, quote = FALSE, na = "") # na = "" so Cytoscape
    write.table(x$nodes, sprintf("nodes_%s_%s_anch.tsv", tag, v),      # reads blanks as missing
                sep = "\t", row.names = FALSE, quote = FALSE, na = "")
  }
}

saveRDS(list(full = net, anchored = sub, pruned = pruned, min_cons = MIN_CONS),
        "network_layers_anch.rds")
...
regulator_status <- function(from, to) {
  ...
  meas_to <- colnames(readRDS(meas_file(to))) # what the other contrast had
...
print(list(shared = intersect(sig_of(pruned$nk), sig_of(pruned$temra)), # common wiring
           ...
           temra_only = setdiff(sig_of(pruned$temra), sig_of(pruned$nk))))


# What did anchoring change?
#
# Same PKN, same solver, same options; the measurement set is the only
# difference. Nodes present here and absent from the baseline solve are those
# the forced HLH regulators recruited.
base_layers <- readRDS("network_layers.rds")

for (tag in tags) {
  a <- net[[tag]]$nodes$id
  b <- base_layers$full[[tag]]$nodes$id
  ...
  cat("TF -> HLH edges: anchored ",
      sum(net[[tag]]$edges$layer == "trn"), ", baseline ",
      sum(base_layers$full[[tag]]$edges$layer == "trn"), "\n", sep = "")
}